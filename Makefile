...
#
# Notes:
#   - Zig 0.16 supplies the FreeBSD CRT/libc. The base sysroot supplies system
#     headers and FreeBSD-only libraries such as Casper and libthr.
...
ifeq ($(FREEBSD),1)
  ...
  # Zig 0.16 supplies the FreeBSD CRT/libc. State the ABI version explicitly so
  # its startup objects match the FreeBSD 15 base used for the remaining system
  # libraries. Target-root -L paths are resolved beneath --sysroot; passing the
  # already-prefixed host paths makes Zig prefix the sysroot a second time.
  CC := $(ZIG) cc -target $(FREEBSD_CPU)-freebsd.$(FREEBSD_ABI) --sysroot=$(AETHER_SYSROOT) -I$(AETHER_SYSROOT)/usr/include
  ...
  FREEBSD_LDFLAGS := -L/usr/lib -L/lib -lthr -lm \
...
COMPILER_SRC = compiler/aetherc.c compiler/parser/lexer.c compiler/parser/parser.c compiler/ast.c compiler/analysis/typechecker.c compiler/analysis/contract_eval.c compiler/analysis/derive.c compiler/analysis/actor_reply.c compiler/analysis/hoist.c compiler/aether_defines.c compiler/codegen/codegen.c compiler/codegen/codegen_expr.c compiler/codegen/codegen_stmt.c compiler/codegen/codegen_actor.c compiler/codegen/codegen_func.c compiler/aether_error.c compiler/aether_module.c compiler/analysis/type_inference.c compiler/codegen/optimizer.c compiler/aether_diagnostics.c compiler/aether_strmap.c runtime/actors/aether_message_registry.c lsp/aether_lsp.c
...
# Every C source with its own main() that no other target compiles: the
# runtime examples, the micro-benchmarks, the demos. Nothing links them, so
# they rot invisibly — three of them had stale include paths and one was
# written against a renamed logging API. `check-standalone` compiles each so
# that cannot happen again.
STANDALONE_SRC = $(wildcard runtime/examples/*.c) \
                 $(wildcard tests/runtime/bench_*.c) \
                 $(STANDALONE_TESTS) \
...
#
# contrib-check-lsan is the second leak gate, for the entries valgrind cannot
# measure: contrib/vulkan renders through lavapipe, whose LLVM JIT makes
# valgrind report ~13k errors, none of them this repo's. LeakSanitizer suppresses
# by module instead, so the driver is excluded by object and every allocation
# the module makes is still gated. Linux only (no LSan on macOS).
.PHONY: contrib-check contrib-check-valgrind contrib-check-lsan
...
contrib-check-lsan: compiler ae stdlib
...
# Syntax-and-warnings gate for the standalone sources. Compile only: several
# are benchmarks that take minutes, and what rots is the code, not the timing.
.PHONY: check-standalone
check-standalone:
	@echo "==================================="
	@echo "  Standalone sources (compile gate)"
	@echo "==================================="
	@fail=0; \
	for f in $(STANDALONE_SRC); do \
	  if $(CC) -fsyntax-only $(AETHER_REQUIRED_CFLAGS) $(CFLAGS) -Werror \
	       -Itests/runtime -Itools/profiler -Istd/log $$f 2>$(BUILD_DIR)/standalone.err; then \
	    printf '  ok    %s\n' "$$f"; \
	  else \
	    printf '  FAIL  %s\n' "$$f"; \
	    head -5 $(BUILD_DIR)/standalone.err | sed 's/^/        /'; \
	    fail=1; \
	  fi; \
	done; \
	rm -f $(BUILD_DIR)/standalone.err; \
	if [ $$fail -ne 0 ]; then echo "check-standalone: FAILED"; exit 1; fi; \
	echo "check-standalone: all standalone sources compile clean"

test-all: test test-ae check-standalone
	@echo ""
	@echo "==================================="
