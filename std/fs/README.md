...
byte on disk and `pread` / `pwrite` through it are byte-exact (C's text-mode
`\r\n` translation never applies). A mode that spells `t` explicitly
(`"wt"`) asks for C's text mode and gets it.
