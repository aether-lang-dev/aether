...
## `--emit=obj`: one object file for an existing C build

`--emit=csrc` hands you generated C that your build then has to know how to
compile. `--emit=obj` does that step for you and stops at the object:

```sh
ae build --emit=obj lib.ae -o lib.o
cc main.c lib.o $(ae cflags --libs) -o app
```

This is what makes an incremental `make` correct without checking generated C
into the tree. A checked-in `*_ae.c` is a stale-artifact trap: edit the `.ae`,
restore the `.c` to keep a diff clean, and the next plain `make` links the old
code with nothing to warn you. With an object rule the `.ae` is the only
source, and the timestamp the build compares is the one that matters:
...
app: main.o lib.o
	$(CC) $^ $(shell ae cflags --libs) -o $@
```

The C compiler used for the object is `$AE_CC`, else `$CC`, else `gcc` when
present, else `cc`, the same resolution order as the other build paths, so a
cross build picks up the same toolchain. Because the object carries the
`aether_<name>()` aliases, a C caller can `#include` a header generated by
`--emit=csrc` (or declare the prototypes itself) and call in directly. Note
that a `-> string` export returns an `AetherString*`, so read its bytes with
`aether_string_data()` rather than treating it as a `char*`.
