...
- **Four cheap, high-value borrows:** `bitstruct` (§5) — the principled fix for a
  bug class that has bitten this repo twice, and **now shipped**,
...
(exclusive) as real lexer tokens, used by match-range case labels. So a
...
> **✅ ADOPTED — see `docs/contract-folding.md` for the costed design and what
> implementation revised.** Two findings worth carrying forward: (1) the
> survey below missed that Aether had already shipped constant-TRUE elision
> as a contracts companion — the §5.5 "what do we already own?" check, applied at
> design time, halved the feature before a line was written; (2) C3-style
> "value-precise" short-circuit folding conflicts with Aether's documented
> guarantee that `x || true` keeps its (possibly side-effectful) runtime
> check, so the shipped evaluator folds `true || x` but not `x || true`.
...
- **Contracts already exist.** `requires` / `ensures` are parsed
  ...
  (`emit_contract_preconditions`), and `LLM.md:335` documents `where`
...
| # | Item | Size | Payoff |
| --- | --- | --- | --- |
| ~~1~~ | ~~**`bitstruct`** (§5)~~ | ✅ **SHIPPED** | Killed the signed-bitfield bug class. Adopted with two divergences from C3 (§5.4, §5.5) |
| ~~2~~ | ~~**`defer catch` / `defer try`** (§6)~~ | ✅ **SHIPPED** | v0.396.0. The groundwork found and fixed a live leak first: `expr!` propagation drained no defers at all |
| 3 | **`@nodiscard`** (§7) | Tiny | Free leak guard |
| ~~4~~ | ~~**Compile-time `where`/contract folding** (§10.2)~~ | ✅ **SHIPPED** | Designed first (docs/contract-folding.md) — which found HALF of it already existed (constant-true elision, already shipped with contracts). Both tiers shipped, error-from-day-one |
...
Items 1, 2 and 4 are done. Of the small independents, `@nodiscard` (§7) and
`attrdef` (§8) remain — apply the §5.5 "what do we already own?" check before
each (RAII overlaps `@nodiscard`'s memory case; `std/`'s shrinking extern
surface weakens `attrdef`'s payoff). Item 6 is the one worth a design
